...
export default async function VendorDetailPage({ params }: VendorPageProps) {
  ...
  let reviewUser: { id: string; name: string; email: string } | null = null;
  if (authUser) {
  ...
  const [
    ...
    supabase
      .from("reviews")
      .select("id, customer_name, rating, comment, event_type, guest_count, is_verified, helpful_count, not_helpful_count, vendor_reply, vendor_reply_at, created_at" as "*")
      .eq("vendor_id", vendor.id)
      .eq("is_approved", true)
