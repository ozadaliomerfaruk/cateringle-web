...
export async function POST(request: NextRequest) {
  try {
    ...
    // Send email notification to vendor (async, don't block response)
    (async () => {
      try {
        // Get vendor info
        ...
        if (vendor?.owner_id) {
          await sendNewReviewEmail({
            ...
            customerName: profile?.full_name || "Anonim",
            reviewId: review.id,
            rating,
            comment: comment?.trim() || null,
            eventType: eventType || null,
            guestCount: guestCount || null,
            reviewDate: review.created_at || new Date().toISOString(),
          });
        }
      } catch (emailError) {
        console.error("Review email notification error:", emailError);
